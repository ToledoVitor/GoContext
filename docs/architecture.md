...
`internal/eval` também é o único seam de execução do harness local. Sem `--gold-set`, ele continua gerando apenas casos automáticos exact-symbol a partir do corpus permitido. Com `--gold-set ABS_PATH`, o composition root abre um arquivo local privado fora da raiz, exige diretório `0700`, arquivo regular owner-only `0600`, um único link e identidade estável antes/depois da leitura, e entrega ao módulo um `GoldSet` opaco. O schema privado 1 exige repository ID e `ScanPolicyVersion` exatos, casos/categorias limitados e julgamentos por `source.Reference`; toda referência precisa resolver para exatamente um chunk canônico já permitido antes de criar a busca. Windows permanece fail-closed para esse input enquanto a mesma garantia de ACL não existir.
