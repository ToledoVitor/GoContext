...
`internal/eval` também é o único seam de execução do harness local. Sem `--gold-set`, ele continua gerando apenas casos automáticos exact-symbol a partir do corpus permitido. Com `--gold-set ABS_PATH`, o composition root abre um arquivo local privado fora da raiz, exige diretório `0700`, arquivo regular owner-only `0600`, um único link e identidade estável antes/depois da leitura, e entrega ao módulo um `GoldSet` opaco. O schema privado 1 exige repository ID e `ScanPolicyVersion` exatos, entre 5 e 1.000 casos, pelo menos cinco casos distintos em cada categoria fornecida e julgamentos por `source.Reference`; toda referência precisa resolver para exatamente um chunk canônico já permitido antes de criar a busca. Windows permanece fail-closed para esse input enquanto a mesma garantia de ACL não existir.

O relatório aggregate-only agora usa schema 2. Categorias positivas publicam Recall@5/10, MRR@10 e nDCG@10 graduado; `negative_evidence` publica apenas `no_evidence_accuracy`. Queries, IDs de caso/chunk, paths, referências, julgamentos, hits e ranks continuam fora do tipo serializável. Artefatos schema 1 permanecem históricos: o comando novo não os migra nem converte dados privados no lugar.
...
`internal/ingest/lineparser` oferece descoberta preliminar enquanto esse spike não ocorre. Reconhece declarações top-level comuns de Python (`def`, `async def`, `class`) e TypeScript (`function`, `class`, `interface`, `type`, `enum`). Para JavaScript/JSX reconhece somente funções nomeadas, inclusive `async` e geradoras, classes nomeadas e uma variável top-level `const`/`let`/`var` atribuída diretamente a arrow function ou function expression, com `export`/`async` apenas nas formas previstas pelo contrato. Uma passagem lexical linear mantém uma pilha única e limitada de blocos, parênteses e colchetes, rejeita fechamentos cruzados e distingue headers/blocos de controle, blocos de statement no início da linha e object literals provados por contexto de expressão. Se `/` segue uma chave que não foi provada como block statement nem object literal, a ambiguidade persiste por comentários/quebra de linha e o restante do arquivo falha fechado; divisão após object literal em atribuição/agrupamento permanece reconhecida. Chaves em comentários, strings, templates e regex reconhecidos não alteram essa pilha. Regiões JSX, inclusive tags multilinha, atributos e substituições simples, permanecem opacas para descoberta de declaração. A validação adicional do arrow body reutiliza o contexto cancelável do caller e mantém checkpoints lineares; varre tokens fora de string, comentário, regex, template raw e texto/tag JSX, entrando somente nas substituições. Parâmetros aceitos são apenas identificadores ASCII simples, únicos e não reservados em módulo estrito; lista incompleta, segmento vazio, destructuring, default, duplicata e nome reservado falham fechado. Arrow concisa precisa começar com um starter de expressão do subconjunto explícito: operandos obrigatórios para unários suportados, `await` em qualquer profundidade somente em arrow sintaticamente `async`, `yield` nunca, prefixos completos de `function`/`class`, JSX completo na mesma linha ou os starters primários/container reconhecidos; block body começa com `{`. A linha só pode produzir símbolo se a passagem completa continuar lexicalmente confiável. Preserva a linha declaratória exata e não inventa nome para default anônimo.

Não interpreta AST, bodies, métodos, símbolos aninhados/indentados, declarações multilinha ou toda a gramática ECMAScript; arrow functions TypeScript continuam fora desse recorte. Formas lexicamente ambíguas ou não modeladas, incluindo `/` após chave sem contexto provado, template aninhado dentro de substituição, JSX aninhado dentro de expressão JSX, JSX conciso incompleto na linha, lista com default/destructuring e trailing comma, propriedade/chave chamada `await`/`yield` ou outro starter de expressão fora do subconjunto, podem suprimir símbolos válidos do restante do arquivo em vez de arriscar boundary inventado. Esses falsos negativos conservadores e as demais limitações impedem apresentar o parser atual como estrutural ou completo.
...
Prova taint ponta a ponta da Task 13, primeiro inventário/baseline lexical profissional e repetição agregada `scanner-v6` da Task 14 estão concluídos. Código profissional não foi a provider externo; somente agregados sanitizados foram versionados em relatório separado da lista nominal de raízes. O harness profissional entregue permanece lexical/offline: mede exact-symbol e deixa conceito/framework/cross-layer como `not-evaluated`. O seam privado de gold set humano existe, mas nenhum conjunto profissional foi executado. Não existe ainda composição aggregate-only profissional para híbrido/Ollama; essa comparação permanece bloqueada pelo gold set humano e por gate verificado de loopback sem telemetry/logging/retenção. Ver [plano de validação](plans/2026-08-27-tivita-professional-repository-validation.md).
