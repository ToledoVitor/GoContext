...
func TestStoreRequiresCurrentScanPolicy(t *testing.T) {
	store := newStore(t)
	legacy, err := source.NewCorpus("scanner-v5", []source.Chunk{sampleChunk("legacy", "legacy.py", "LEGACY_POLICY_CANARY")})
	if err != nil {
...
func TestStoreRejectsPersistedStalePolicyRevisionAndContent(t *testing.T) {
	tests := []struct {
		...
		{name: "stale policy", mutate: `UPDATE generations SET scan_policy_version = ?`, argument: "scanner-v5"},
